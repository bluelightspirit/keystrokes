...
### Cloning this repository (```git clone https://github.com/bluelightspirit/windows-macros```), opening the project through the .sln file (```windows-macros.sln``` which may be in the windows-macros subdirectory after cloning), then running ```Program.cs``` has worked when bluelightspirit tested this on an external computer.
